...
def add_info(df, path):
    sirie = pd.read_excel(f'{path}/Цены на сырье.xlsx')
    ...
    gruz = pd.read_excel(f'{path}/Грузоперевозки.xlsx')
    ...
    metall = pd.read_excel(f'{path}/Показатели рынка металла.xlsx')
    ...
    materials = pd.read_excel(f'{path}/Данные рынка стройматериалов.xlsx') # То же, пока что только по месяцам, надо по дням
    ...
    # То же самое
    toplivo = pd.read_excel(f'{path}/Топливо.xlsx')
    ...
    # И опять:)
    lme = pd.read_excel(f'{path}/Индекс LME.xlsx').rename({'дата':'dt', 'цена': 'индекс lme'}, axis=1).dropna()
...
def make_prediction(path_model='models.pkl', path_features='data', path_output='output.xlsx', date='2023-03-13'):
    models, month_stats = pickle.load(open(path_model, 'rb'))

    train = pd.read_excel(f'{path_features}/train.xlsx')
    test = pd.read_excel(f'{path_features}/test.xlsx')
    all_df = pd.concat((train, test)).reset_index(drop=True)
    all_df = add_info(all_df, path_features)
    all_df['Изменение в месяце'] = all_df['month'].apply(lambda x: month_stats[x])
    
    date = date.split('-')
    date = [int(x) for x in date]
    
    index = all_df[(all_df.dt.dt.year==date[0])&(all_df.dt.dt.month==date[1])&(all_df.dt.dt.day==date[2])].index[0]
    cur_df = make_features(all_df.iloc[:index+1])
    pred = [x[0].predict_proba(cur_df[x[1]])[1] for x in models]
    p = 1
    for x in pred:
        if x > 0.5:
            p += 1
